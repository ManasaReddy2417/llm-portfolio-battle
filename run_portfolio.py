...
var lineTraces = [];
'''
    # Small visual display offsets to separate clustered ~$1000 lines
    _display_offsets = {'ChatGPT': 25, 'Grok': -25}
    for i, (llm_name, data) in enumerate(ranked):
        cv = data['cumulative_values']
        _off = _display_offsets.get(llm_name, 0)
        cv_disp = [round(v + _off, 2) for v in cv] if _off else cv
        html += f'''
lineTraces.push({{
  x: dates, y: {_json.dumps(cv_disp)},
...
'''
    ...
    for i, (llm_name, data) in enumerate(ranked):
        ...
        last_val = cv[-1]
        _off     = _display_offsets.get(llm_name, 0)
        last_disp = last_val + _off   # Y position matches the displayed line
        trp      = data['total_return_pct']
        ssym     = '+' if trp >= 0 else ''
        label    = f'{llm_name}: ${last_val:.0f} ({ssym}{trp:.1f}%)'
        all_series.append((last_disp, label, colors[i]))
...
for (var li=0; li<labelArr.length; li++) {{
  var y = labelArr[li][0];
  for (var lj=0; lj<adjY.length; lj++) {{ if (Math.abs(adjY[lj]-y) < 35) y = adjY[lj]-35; }}
  adjY.push(y);
  annotations.push({{
