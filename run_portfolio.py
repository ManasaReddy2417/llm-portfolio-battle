  ...
  <div class="section">
    <div class="section-title" style="justify-content:space-between;flex-wrap:wrap;gap:10px;">
      ...
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        <label for="baseline-select" style="font-size:.85em;font-weight:700;color:#1e3a8a;white-space:nowrap;">📅 Baseline:</label>
        <select id="baseline-select" onchange="switchBaseline(this.value)" style="font-family:Inter,sans-serif;font-size:.88em;font-weight:600;color:#1e3a8a;background:#fff;border:2px solid #6366f1;border-radius:8px;padding:6px 12px;cursor:pointer;outline:none;box-shadow:0 1px 4px rgba(99,102,241,.15);">
          <option value="feb" selected>Feb 1 &mdash; Full history</option>
          <option value="may">May 1 &mdash; Post-DeepSeek</option>
        </select>
      </div>
    </div>
...
function switchBaseline(val) {{
  ...
  if (val === 'may') {{
    renderChart(mayData);
    if (titleEl) titleEl.innerHTML = '&#128198; Cumulative Portfolio Value vs S&amp;P 500 (May 1 &ndash; present, rebased to $1,000)';
  }} else {{
    renderChart(febData);
