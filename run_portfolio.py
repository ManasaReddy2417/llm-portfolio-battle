...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  body{font-family:'Inter','Segoe UI',sans-serif;background:linear-gradient(135deg,#6366f1 0%,#818cf8 30%,#a78bfa 60%,#7c3aed 100%);min-height:100vh;padding:0 80px 30px;}
...
'''
    for i, (llm_name, data) in enumerate(ranked):
        ...
        # Re-normalize so chart day-0 is exactly $1000 for every series
        cv0 = cv[0] if cv[0] else INITIAL_INVESTMENT
        cv_norm = [round(v / cv0 * INITIAL_INVESTMENT, 2) for v in cv]
        html += f'''
lineTraces.push({{
  x: dates, y: {_json.dumps(cv_norm)},
...
'''
    ...
    for i, (llm_name, data) in enumerate(ranked):
        cv       = data['cumulative_values']
        cv0      = cv[0] if cv[0] else INITIAL_INVESTMENT
        last_val = round(cv[-1] / cv0 * INITIAL_INVESTMENT, 2)   # normalized last value
        trp      = data['total_return_pct']
        ssym     = '+' if trp >= 0 else ''
        label    = f'{llm_name}: ${cv[-1]:.0f} ({ssym}{trp:.1f}%)'
...
// Sort descending by actual Y value
labelArr.sort(function(a,b){{ return b[0]-a[0]; }});
var adjY = [];
var MIN_GAP = 38;  // minimum pixels between label centers
for (var li = 0; li < labelArr.length; li++) {{
  var y = labelArr[li][0];
  // Push down if too close to any already-placed label above
  for (var lj = 0; lj < adjY.length; lj++) {{
    if (adjY[lj] - y < MIN_GAP) {{ y = adjY[lj] - MIN_GAP; }}
  }}
  adjY.push(y);
  annotations.push({{
