...
// ── Spread labels so none overlap — iterative push-apart ──────────
var MIN_GAP = 26;
var adjY = labelArr.map(function(s){{ return s[0]; }});
for (var pass=0; pass<80; pass++) {{
  var moved = false;
  for (var li=0; li<adjY.length-1; li++) {{
    var gap = adjY[li] - adjY[li+1];
    if (gap < MIN_GAP) {{
...
for (var li=0; li<labelArr.length; li++) {{
  var actualY  = labelArr[li][0];
  var labelY   = adjY[li];
  var needArrow = Math.abs(labelY - actualY) > 10;
  annotations.push({{
    x: lastDate, y: labelY,
    xanchor: 'left', yanchor: 'middle', xshift: 12,
    text: '<b>'+labelArr[li][1]+'</b>',
    showarrow: needArrow,
    ax: needArrow ? -8 : 0,
    ay: needArrow ? (actualY - labelY) * 0.6 : 0,
    arrowhead: 0, arrowwidth: 1.5,
    arrowcolor: labelArr[li][2],
    font: {{ size: 13, color: labelArr[li][2], family: 'Inter,sans-serif' }},
    bgcolor: 'rgba(255,255,255,0.95)', borderpad: 4,
    bordercolor: labelArr[li][2], borderwidth: 1
  }});
}}
